...
#define RAND(min, max) ((min) + (float)rand()/(RAND_MAX) * ((max)-(min))) // RAND_MAX+1 dava warning: integer overflow in expression of type ‘int’ results in ‘-2147483648’ [-Woverflow]
...
osg::Node* createObject( const std::string& filename, const osg::Vec4& color )
{
	...
	if ( model_node.valid() )
		model_node->setNodeMask( MAIN_CAMERA_MASK );
...
int main( int argc, char** argv )
{
	...
	osg::Node* obj2 = createObject( "dumptruck.osg.0,0,180.rot", osg::Vec4(0.2f, 1.0f, 0.2f, 1.0f) ); // troquei pra verde
